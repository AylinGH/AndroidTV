    ...
    private fun loadDeatails(item: ObjectSerializable) {
        DoAsync(object : CompletedListener {
            ...
            if (item.obj is MovieDb) {
                obj = tmdb!!.movies.getMovie((item.obj as MovieDb).id, "en-US", TmdbMovies.MovieMethod.videos)
                ...
                val c = tmdb!!.tvSeries.getSeries((item.obj as TvSeries).id, "en", TmdbTV.TvMethod.external_ids, TmdbTV.TvMethod.content_ratings, TmdbTV.TvMethod.videos)
                val lastSeason = tmdb!!.tvSeasons.getSeason(c.id, c.seasons.last().seasonNumber, "en", TmdbTvSeasons.SeasonMethod.external_ids)
